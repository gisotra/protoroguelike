...
@onready var new_point_floor: Vector2
...
func _ready():
	...
	death_timer.start()
	new_point_floor.x = randf_range(get_parent().position.x - 50, get_parent().position.x + 50)
	new_point_floor.y = randf_range(get_parent().position.y - 50, get_parent().position.y + 50)
